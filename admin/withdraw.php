<?php 
  ...
  $sql = "SELECT *, COALESCE(ROUND(SUM(order_total_net), 2), 0) as sumTotal FROM orders WHERE order_status = 'successful'";
  $result = $conn->query($sql);
  $row = $result->fetch_assoc();

  $sql = "SELECT COALESCE(SUM(withdraw_money), 0) as withdrawPending, COALESCE(SUM(withdraw_fee), 0) as withdrawFeePending FROM withdraw WHERE withdraw_status = 'pending' OR withdraw_status = 'wait_confirm'";
  $result = $conn->query($sql);
  $row2 = $result->fetch_assoc();

  $sql = "SELECT COALESCE(SUM(withdraw_money), 0) as withdrawSuccessful, COALESCE(SUM(withdraw_fee), 0) as withdrawFeeSuccessful FROM withdraw WHERE withdraw_status = 'successful'";
  $result = $conn->query($sql);
  $row3 = $result->fetch_assoc();

  $sql = "SELECT COALESCE(SUM(withdraw_fee), 0) as withdrawFee FROM withdraw WHERE withdraw_status = 'successful'";
  $result = $conn->query($sql);
  $row4 = $result->fetch_assoc();

  $sql = "SELECT * FROM withdraw LEFT JOIN seller ON withdraw.seller_id = seller.seller_id";
  $result = $conn->query($sql);
?>
  ...
  <div class="row">
    ...
    <div class="col-xl-3 col-md-6">
      <div class="widget widget-stats bg-teal">
        ...
        <div class="stats-content">
          <div class="stats-title">ยอดเงินคงเหลือทั้งหมด</div>
    ...
    <div class="col-xl-3 col-md-6">
      <div class="widget widget-stats bg-success">
        <div class="stats-icon stats-icon-lg"><i class="fa fa fa-coins fa-fw"></i></div>
        <div class="stats-content">
          <div class="stats-title">ค่า fee ที่ได้รับ</div>
          <div class="stats-number"><?php echo number_format(intval($row4['withdrawFee']*100)/100, 2); ?></div>
        </div>
      </div>
    </div>
    <!-- end col-3 -->
      ...
      <table id="data-table-default" class="table table-bordered">
        <thead>
          <tr>
            ...
            <th class="text-nowrap">ชื่อผู้ใช้</th>
            <th class="text-nowrap">ชื่อ-สกุลผู้ขาย</th>
            <th class="text-nowrap">เลขที่บัญชี</th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?php echo $row['seller_payment']; ?></td>
            ...
            <td class="text-center">
              <button class="btn btn-warning" onclick="if(confirm('ยืนยันการโอนเงิน?')){ location.href = 'process_withdraw.php?id=<?php echo substr(md5($row['withdraw_id']), 0, 8); ?>&action=update' }" <?php echo ($row['withdraw_status']!='pending')?"disabled" : ""; ?> >ยืนยันการโอนเงิน</button>
            </td>
          </tr>
          <?php } ?>
        </tbody>
